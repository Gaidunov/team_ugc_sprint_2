...
def pg_upload_review_data(data:list):
    ...
    review_data = tuple( (rev['review_id'], rev['text'], rev['movie_id'], ) for rev in data) 
    meta_data =   tuple( ( (rev['author_id'],), rev['author_name'], rev['date'], rev['review_id'] ) for rev in data)
    try:
